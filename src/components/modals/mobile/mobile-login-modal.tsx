...
export function MobileLoginModal({ show }: { show: boolean }) {
  ...
  // The drawer stays mounted while closed (only `inert`), so any <h1> here
  // would be counted in the page outline and produce 3 H1s (QA BUG-12).
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 text-left">
          <h2 className="mb-6 text-[28px] inter-light tracking-tight">
            {t("login.welcomeTitle")}
          </h2>
          <div className="inter-light text-base text-left">
            <span>{t("login.welcomeNotMember")} </span>
            <button
              type="button"
              onClick={() => showRegister()}
              className="inter-light text-white underline transition-opacity hover:opacity-80"
            >
      ...
      <div className="relative z-[5] -mt-5 bg-white px-6 pb-20 pt-8"
        ...
        <div className="mx-auto max-w-md">
          <h2 id={titleId} className="mb-8 text-[28px] inter-medium text-[#29221D]">
            {t("login.title")}
          </h2>
