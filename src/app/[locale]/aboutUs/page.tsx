...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata(validLocale, "aboutUs");
}
...
export default async function AboutPage({ params }: Props) {
  ...
  return (
    <>
      {/* Organization + WebSite come from the locale layout; the route only
          owes a BreadcrumbList (QA BUG-02). */}
      <PageJsonLd page="aboutUs" />
      <AboutContent />
    </>
  );
}
